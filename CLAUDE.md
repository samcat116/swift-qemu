...
21. **Typed Throws and `Duration`** (issue #23, source-breaking): the whole public surface is now `throws(QMPError)`, and every timeout is a `Duration` rather than a bare `TimeInterval` of seconds. Both are breaking, which is why they landed together.
    ...
    - **`QMPWaiter` is typed at the latch**, not at its callers: `Result<Value, QMPError>`, so `resolve` cannot be handed a foreign error in the first place. Only `withTaskCancellationHandler` still `rethrows` as `any Error`, so `value` restores the domain once on the way out
