...
20. **Typed Throws and `Duration`** (issue #23, source-breaking): the whole public surface is now `throws(QMPError)`, and every timeout is a `Duration` rather than a bare `TimeInterval` of seconds. Both are breaking, which is why they landed together.
    ...
    - The continuations in `QMPChannelHandler` stay `CheckedContinuation<_, any Error>` — the stdlib has no typed-throws variant — so each of the three waits converts once, at its own boundary
...
fix 20 for what that cost, and note in particular that **cancellation arrives as
