...
The suite is **Swift Testing**, not XCTest — `@Suite`/`@Test`/`#expect`, with
`.timeLimit` traits bounding the hang regressions and `.enabled(if:)` skipping the
tests that need a real QEMU (see fix 22). `--filter` matches the suite type name
the same way it did, so the commands above are unchanged.
...
- Actor-based for thread-safe concurrent access — but an actor serializes statements, not methods. Anything guarding shared state here has to be checked and set with **no `await` in between**, or a second call re-enters the method across the suspension and walks straight past it: `createVM`'s `isCreating` and `attachDisk`'s port claim are both that shape, and were both bugs before (see fix 23)
...
- Handles QMP Unix socket creation and readiness with retry logic (up to 10 seconds, polled on a doubling interval — see fix 23)
...
- Handles the QMP greeting, capability negotiation, and command execution
- Executes QMP commands: query-status, cont, stop, system_powerdown, system_reset, quit, query-yank, yank, and the block hot-plug set
...
- Holds the current `QMPConnection`, the task consuming it, and what negotiation settled on, in one lock-guarded box — so the type is honestly `Sendable` rather than `@unchecked Sendable`
...
- `disconnect()` is idempotent, treats an already-closed channel as success, and waits for the reader task — so everything parked on the connection has been failed, and every event stream finished, by the time it returns
- Builds the pipeline for each connection: `ByteToMessageHandler(QMPFrameDecoder(...))` then a `NIOAsyncChannel<ByteBuffer, ByteBuffer>` (see fix 20)
...
- An `actor` owning one live connection: the loop consuming the inbound `AsyncSequence`, the outstanding waiters, and the write side. Replaces the old `QMPChannelHandler` (see fix 20)
...
- Correlation is unchanged: replies are matched on the `id` this client generates, a tagged reply with no waiter is discarded rather than shifted onto another caller, and untagged replies (old QEMU) fall back to submission order among requests that have actually reached the socket. `outOfBand` is carried across the rebuild that adds the id, or an OOB command silently goes out in-band
...
- **One rule: the first resolution wins, and a caller that parks afterwards resumes immediately with what was latched.** Delivery, deadline, cancellation and teardown are four callers of `resolve`, in any order, from any thread, so there is no "install the waiter first, arm the deadline second" ordering left to get wrong (see fix 20)
- The greeting waiter carries the `QMPGreeting` itself, so negotiation reads the capabilities out of the wait rather than fetching them from somewhere else afterwards

**QMPEventBroadcaster** (Sources/SwiftQEMU/QMPEventBroadcaster.swift)
- The per-connection fan-out behind `events(bufferSize:)`: one `AsyncStream` continuation per subscriber, `bufferingNewest` so a yield can never block the reader loop, finished when the connection ends
- Lock-guarded rather than actor-isolated, because subscribing must stay **synchronous** — an `async` `events()` would open a window for an event to slip past between the call and the first iteration
...
- Enforces `maximumFrameSize` (default 512 KB) on the frame itself, terminating newline included; over-limit frames fail the connection with `QMPError.frameTooLarge` (see fix 15). This is **not** `ByteToMessageHandler(_:maximumBufferSize:)`, which cannot express it — see fix 20
...
2. **QMP Connection Retry Logic**:
   - QEMUProcess waits up to 10 seconds for QMP socket file creation, polling on a doubling interval from 10ms to 250ms (see fix 23) rather than on a fixed 0.5s tick
...
12. **`QEMUProcess` Isolation**: `QEMUProcess` was a `final class` marked `@unchecked Sendable` while holding four unsynchronized mutable fields (`process`, `stderrPipe`, `stderrCapture`, `exitWaiter`). `createVM` touches it from two concurrency domains at once — a task-group child runs `start(with:)`, which writes all four, while the failure path reads `capturedStderr`/`isRunning`. The annotation was the only reason Swift 6 did not diagnose it; the inner `StderrCapture`/`ExitWaiter` locks protect their contents, not the fields referencing them. It is now an `actor`, and `QEMUProcessTests.statusCanBeReadConcurrentlyWithAStartInFlight` covers the overlap — under `swift test --sanitize=thread` it reported races on the `stderrCapture` and `process` writes against a class and none against the actor.
    ...
    - `stop()` being `await`-ed is what put the tests' cleanup out of reach of `defer`. Under XCTest that meant `addTeardownBlock`; Swift Testing has no async teardown hook at all, so the cleanup is now either a scope (`withVM`) or `deinit` — see fix 22
...
15. **Bounded Inbound Frames**: `channelRead` accumulated everything it read and only drained on a newline, so a peer that never sent one grew the buffer for as long as it kept writing. Fix 6's `discardReadBytes()` stopped *consumed* frames accumulating; a single unterminated frame was still unbounded. `maximumFrameSize` (default `QMPClient.defaultMaximumFrameSize`, 512 KB, settable per client) now caps one frame including its terminating newline, and exceeding it fails the connection with `QMPError.frameTooLarge(limit:)`.
    ...
    - 512 KB is far above any real QMP message (`query-block` on a long device list, the largest realistic payload, runs to tens of KB). `QMPClientTests`' "A large but legal frame is delivered" exists because a cap that clipped legitimate payloads would be worse than no cap
    - The cap now lives in `QMPFrameDecoder`, and deliberately **not** in `ByteToMessageHandler(_:maximumBufferSize:)` — see fix 20 for why that knob cannot express it. Fix 19 also removed the ordering rule this fix used to need: waiters were failed *before* the close so the in-flight caller got `frameTooLarge` rather than the `connectionLost` `channelInactive` would latch a moment later, and only one place notices a connection ending now
...
18. **Events Reach Callers, and the Greeting's Capabilities Are Used**: events were decoded correctly (fix 6) but only `DEVICE_DELETED` was acted on — everything else was logged at debug level and dropped, so `QEMUManager.status` went stale until someone called `getStatus()` and a guest powering itself off was invisible. The greeting's `capabilities` were decoded and discarded in the same way, leaving the protocol's only optional feature permanently off. Now:
    - `QMPClient.events(bufferSize:)` returns an `AsyncStream<QMPEvent>`. **Per subscriber**, so the manager's own bookkeeping and an application's `for await` do not displace each other; **bounded** at `bufferSize` (default 64) keeping the *newest*, because a blocking yield would stall the loop that also delivers command replies and so wedge QEMU's socket for every waiter; and **finished when the connection ends**, so a `for await` terminates instead of parking. A stream belongs to one connection — reconnecting means resubscribing. Since fix 20 the fan-out is `QMPEventBroadcaster` and the yields happen on the reader loop rather than the event loop; the requirement is the same either way
    - `DEVICE_DELETED` keeps its dedicated ticket path (a detach needs a targeted, timed wait, not a scan of a shared stream) *and* is published to subscribers. `QMPClientTests.deviceDeletedReachesBothItsTicketAndTheStream` is the guard on that
...
20. **The Transport Runs on `NIOAsyncChannel`**: `QMPChannelHandler` hand-rolled two things NIO provides — newline framing over an accumulating `ByteBuffer`, and a registry of `CheckedContinuation`s bridging the event loop to async callers. That registry is where fixes 6, 10, 14 and 15 all landed, one guard at a time, because every wait re-implemented the same ordering problem separately. It is now four pieces: `QMPFrameDecoder` (a `ByteToMessageDecoder`), `QMPConnection` (an actor consuming a `NIOAsyncChannel`'s inbound `AsyncSequence`), `QMPWaiter` (one latch, used by all three kinds of wait), and `QMPEventBroadcaster` (fix 18's event fan-out, lifted out of the handler unchanged in behaviour). The handler is gone, and with it ~350 lines of per-waiter state machine.
    ...
    - **`NIOExtras.LineBasedFrameDecoder` was evaluated and rejected**, contrary to what issue #21 assumed. Two findings: (a) its `maximumBufferSize` does *not* subsume fix 15 — that cap is checked only on bytes the decoder refused to consume (`Codec.swift`, `decoderResult == .needMoreData`), so a *complete* over-limit frame is decoded and delivered before the check ever runs, and `completeButOversizedFrameFailsTheConnection` would fail; a downstream size check would be needed anyway. And (b) swift-nio-extras 1.32 pulls in 13 transitive packages — swift-nio-ssl, swift-nio-http2, swift-certificates, swift-crypto, swift-service-lifecycle among them — which is a steep graph for a library that talks to a local process over a Unix socket. `QMPFrameDecoder` is ~30 lines, states the cap exactly, and `ByteToMessageHandler` still owns the buffer, the partial-frame carry-over and the reclaiming
...
22. **The Suite Is Swift Testing** (issue #24): all nine files moved off XCTest in one pass rather than incrementally — a half-migrated target means two cleanup idioms and two skip mechanisms side by side, which is worse than either. No `XCTest` import remains, and `Package.swift` needed no change: swift-testing ships with the toolchain and SwiftPM links it into a test target on its own. Every assertion came across; the count went from 154 `func test…` to 147 tests, because parameterization merged tables that were previously separate cases (run states, event names, accelerators, machine types, `JSONValue` accessors, both frame-limit budgets, both `DEVICE_DELETED` orderings, and the zero-plus-tiny timeout pairs).
    - **`.timeLimit` is a hang backstop, not a budget, and the distinction is load-bearing.** `TimeLimitTrait.Duration` offers nothing below `.minutes(_:)` — the `seconds`/`milliseconds` spellings exist but are marked `@available(*, unavailable, message: "Time limit must be specified in minutes")` — so `.hangBackstop` (1 minute) and `.slowHangBackstop` (2, for real VMs) can only say "this stopped making progress". The two tests the issue expected to *replace* with a trait therefore keep measuring: an early exit has to beat the **10s** socket wait, and a cancelled `waitUntilExit()` has to beat a **120s** child. A minute-granularity limit would pass in both regressions. What the traits do buy is the failure mode CI cares about — a stuck run becomes a failed test — and they now cover every wait, not just the two that were hand-timed. The measurements moved from `Date()` to `ContinuousClock` while they were being touched, for fix 21's reason
    - **Asynchronous teardown has no hook at all.** XCTest's `addTeardownBlock` could await; Swift Testing's per-test instance gives `init`/`deinit`, and a `deinit` cannot. So cleanup split by what it needs: paths go in `TemporaryFiles`, whose own `deinit` removes them when the suite instance is released, and anything awaited is bounded by a scope — `withVM` for a real VM, `withServer`/`withConnectedClient` for the fake QMP server. The scopes are strictly better than what they replace: `defer { Task { await server.shutdown() } }` fired off cleanup nobody waited for, and the trailing `try await client.disconnect()` calls were skipped on exactly the failure paths that most needed them
    - **`QEMUProcessTests` deliberately relies on `QEMUProcess.deinit`** instead of wrapping thirty tests in a scope: dropping a process SIGKILLs a live child and removes its socket and private directory (fix 11), so falling out of scope *is* the cleanup, including on a failure path. Tests that are about stopping still call `stop()`, because a graceful stop is what they assert. Verified rather than assumed — a full run leaves no `fake-qemu` children and nothing matching `qemu-*` in the temporary directory
    - **`FakeQMPServer`'s lifetime is the scope and not ARC's**, which is not a stylistic preference: releasing it closes the listening socket, and `connectUnix` retries. A server collected between attempts turns the negotiation `.timeout` that `silentPeerKeepsItsNegotiationErrorRatherThanBeingWrapped` asserts into a `.connectionFailed`. It moved to its own file in the process, since it is no longer nested in an `XCTestCase`
    - **`QEMUProcessTests` is `.serialized`**, which is what XCTest gave it for free. `logFileIsPrivateAndOutlivesTheVM` sets `ENABLE_QEMU_PROCESS_LOG_FILES`, and that is process-global: run in parallel with `privateSocketDirectoryIsRemovedOnStop`, which asserts a directory is gone, it keeps the directory alive and fails it. Parallelism would cut that suite to roughly a third, and is not worth a race in the suite whose subject is process lifecycle. The residual window is cross-suite — a real-QEMU suite starting a process while the variable is set costs it a leftover instance directory, which nothing asserts on and nothing in-process can prevent, because the variable is read from the environment because that *is* the API. Everything else runs in parallel: the full suite is ~19s, essentially all of it that one serialized file (it was ~32s until fix 23 stopped charging every process start most of a second in socket polling)
    - **`throw XCTSkip` became `.enabled(if:)`** (`.requiresQEMU`, `.requiresQEMUAndImageTool`), so a machine without QEMU decides before the test body runs rather than after it has built fixtures for a test that was never going to run. The real-QEMU tests split into their own suites (`RealQEMUConfigurationTests`, `RealQEMUVMStatusTests`, `RealQEMUQMPClientTests`, `RealQEMUHotplugTests`) because that is where the trait belongs — a suite-level condition, not a line in each test
    - `#expect(throws: QMPError.self)` returns the error, so `do { … XCTFail } catch { guard case }` collapsed to `let error = try await #require(throws:)` followed by the same `guard case`. `QMPError` is not `Equatable` — `underlying(any Error)` — so pattern matching stays; `Issue.record` replaces `XCTFail` in the `else`
    - `swift test --sanitize=thread` still reports no races across the whole suite, now with tests running concurrently, which is a stronger statement than the serial XCTest run made
    - `--filter` matches suite type names as before, so every command in this file still works

23. **Actor Reentrancy in `QEMUManager`, and a Socket Wait That Charged Every Start** (performance and correctness review): three findings, two of them the same bug in two places. `QEMUManager` is an actor, which serializes its *statements* but not its *methods* — a method that suspends is re-entered, and both of these guarded shared state across an `await`.
    - **`createVM` guarded on `await process.isRunning`, which is a suspension point**, so two concurrent calls both got past it. The loser then failed on `QEMUProcess`'s own `processAlreadyRunning` guard *inside* the task group, which put it on the failure path — and that path calls `process.stop()` and `forgetHotplugState()`, neither of which belonged to it. Measured rather than reasoned about: with the fix reverted, `aRefusedConcurrentCreateDoesNotTearDownTheWinnersVM` reports *both* calls failing and the VM destroyed, because the loser's cleanup killed QEMU while the winner was still connecting to its socket. The guard is now `isCreating`, tested and set with no `await` in between, which is the only thing that makes it a guard at all
    - **`attachDisk` chose a port and claimed it on opposite sides of two QMP round-trips.** Choosing late was deliberate — a rejected attach should not burn a port — but across those suspensions a second attach saw the first's port as still free and took it, and QEMU refused it with `PCI: slot 0 function 0 already occupied`. That is a bare `GenericError`, which `hotplugFailure` does not rewrite, so the pool's whole purpose was defeated exactly when it was being exercised. Now claimed up front by `claimHotplugBus` (synchronous, `await`-free by construction) and given back on every failure path, so the original property survives without the race
    - `HotplugPortPool` gained `release(_:from:)` and a no-overwrite rule in `claim`. Releasing by device name alone would hand back a port whose device is still in it, in the one case where an attach claims nothing: a device name that already holds a port. Overwriting in `claim` would orphan the first port — a slot physically occupied that the pool reports as free, which outlives the duplicate `device_add` QEMU is about to refuse
    - **The socket wait polled on a fixed 500ms tick and then slept 200ms more** once the file appeared. QEMU creates the socket in a few tens of milliseconds, so the tick was mostly quantization and the 200ms was covering for a socket that exists but is not yet accepting — which `QMPClient.connectUnix` already retries with its own backoff. Both were charged to every VM this library has ever started. Now a `ContinuousClock` deadline with a doubling poll (10ms → 250ms, same 10s budget) and no settling sleep: `swift test --filter QEMUProcessTests` went from **32.1s to 18.2s**, measured on the same machine with only these changes stashed in and out
    - The two behavioural fixes are covered by tests that were confirmed to fail without them (`aRefusedConcurrentCreateDoesNotTearDownTheWinnersVM`, `concurrentAttachesTakeDifferentPorts`), which is the only thing that makes a regression test one. `aFailedAttachGivesItsPortBack` guards the property claiming-late used to give for free. Suite is 152 tests, still no races under `--sanitize=thread`

### Known Gaps

Reviewed and deliberately left for follow-up work — do not assume these are handled:

- **Thin end-to-end `QEMUManager` coverage**: most of its bugs were regression-tested at the `QMPClient`/`QEMUProcess` level. `RealQEMUVMStatusTests` and `RealQEMUHotplugTests` drive the manager against a real QEMU for the create → start → attach → destroy paths (skipped where QEMU is absent); covering the failure paths still needs an injectable QMP-speaking fake, since `QMPClient` is created in `init`
...
- QEMUProcess is an actor. Its child-process state is genuinely shared across domains, which is the reason it is isolated rather than annotated (see fix 12). The detached task running `Subprocess.run` reaches it only through the locked `ChildProcess`
- QMPClient is `Sendable`, holding the current connection and its reader task in one lock-guarded box
- QMPConnection is an actor. Inbound messages arrive as an `AsyncSequence` from `NIOAsyncChannel`, so the registries are touched only from isolated methods and there is no lock at this layer (see fix 20)
- QMPWaiter and QMPEventBroadcaster hold the only locks left in the QMP layer, and for stated reasons: a waiter is resolved from event-loop timers as well as from tasks, and subscribing to events has to stay synchronous
- All async operations use Swift's async/await and structured concurrency

### Dependencies

- swift-nio: Async networking for QMP socket communication (`NIOCore`, `NIOPosix`; `NIOEmbedded` for tests only). `swift-nio-extras` was evaluated for framing and rejected — see fix 20
...
`RealQEMUConfigurationTests.defaultConfigurationStartsRealQEMU` starts a real QEMU with the stock configuration when one is installed, and skips otherwise. It is the only test that can confirm the accelerator and CPU model are actually accepted.

`RealQEMUHotplugTests` goes further and drives `QEMUManager` end to end against a real QEMU — create, `attachDisk`, `query-block`, destroy — because whether a `device_add` lands anywhere is a fact about QEMU's PCI topology that no fake can answer. It needs `qemu-img` as well (each test hot-plugs a real 16M qcow2) and skips when either binary is missing. Each VM boots paused with 128MB under TCG, so the whole suite is under a second per VM.

Skipping is a `.enabled(if:)` trait on those suites rather than a `throw` in each body (see fix 22), so what a machine without QEMU can run is decided before any fixture is built. `QEMUFixtures` in `Tests/SwiftQEMUTests/TestSupport.swift` is the one place the binary paths are looked up.
...
- `RealQEMUVMStatusTests.createdVMReportsPausedAndBecomesRunningOnStart` — the run state comes from QEMU, not the test
- `RealQEMUVMStatusTests.eventsFromARealQEMUReachTheManagersSubscribers` — real event names and ordering (`RESUME` then `STOP`), and that `oob` negotiation succeeds
- `RealQEMUQMPClientTests.outOfBandCommandIsAcceptedByARealQEMU` — an `exec-oob` request QEMU actually answers, which is the only check that would have caught the rejected `control` spelling
...
Three layers below `QMPClientTests`, deliberately separate:
...
The fake server itself is `Tests/SwiftQEMUTests/FakeQMPServer.swift`, and its lifetime is a scope — `withServer` / `withConnectedClient` in `QMPClientTests`. That is not tidiness: releasing it closes the listening socket, and `connectUnix` retries, so a server collected between attempts changes which error a test sees. Add a behaviour to `FakeQMPServer.Behaviour`, never a second server.

The concurrency claims are checked with the sanitizer, which must report no races — now with the suites running in parallel, which XCTest did not do:
