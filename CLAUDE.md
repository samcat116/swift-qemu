...
22. **The Suite Is Swift Testing** (issue #24): all nine files moved off XCTest in one pass rather than incrementally — a half-migrated target means two cleanup idioms and two skip mechanisms side by side, which is worse than either. No `XCTest` import remains, and `Package.swift` needed no change: swift-testing ships with the toolchain and SwiftPM links it into a test target on its own. Every assertion came across; the count went from 154 `func test…` to 147 tests, because parameterization merged tables that were previously separate cases (run states, event names, accelerators, machine types, `JSONValue` accessors, both frame-limit budgets, both `DEVICE_DELETED` orderings, and the zero-plus-tiny timeout pairs).
    ...
    - **`QEMUProcessTests` is `.serialized`**, which is what XCTest gave it for free. `logFileIsPrivateAndOutlivesTheVM` sets `ENABLE_QEMU_PROCESS_LOG_FILES`, and that is process-global: run in parallel with `privateSocketDirectoryIsRemovedOnStop`, which asserts a directory is gone, it keeps the directory alive and fails it. Parallelism would cut that suite from ~32s to roughly a third of it, and is not worth a race in the suite whose subject is process lifecycle. The residual window is cross-suite — a real-QEMU suite starting a process while the variable is set costs it a leftover instance directory, which nothing asserts on and nothing in-process can prevent, because the variable is read from the environment because that *is* the API. Everything else runs in parallel: the full suite is ~32s, essentially all of it that one serialized file
